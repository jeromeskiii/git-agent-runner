...
class PRDescription:
    ...
    async def run(self):
        try:
            ...
            get_logger().exception(f"Error generating PR description {self.pr_id}",
...
"""
            ...
            get_logger().exception(f"Error extending additional files {self.pr_id}")
    ...
    def _prepare_labels(self) -> List[str]:
        ...
        try:
            ...
            get_logger().exception(f"Error converting labels to original case {self.pr_id}")
        return pr_labels
    ...
    def _prepare_pr_answer_with_markers(self) -> Tuple[str, str, str, List[dict]]:
        ...
        if ai_walkthrough and not re.search(r'<!--\s*pr_agent:walkthrough\s*-->', body):
            try:
                ...
                get_logger().exception(f"Failing to process walkthrough {self.pr_id}")
