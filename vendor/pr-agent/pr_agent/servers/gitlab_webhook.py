...
def is_bot_user(data) -> bool:
    try:
        ...
        get_logger().exception(f"Failed 'is_bot_user' logic")
    return False
...
def is_draft(data) -> bool:
    try:
        ...
        get_logger().exception(f"Failed 'is_draft' logic")
    return False
...
def is_draft_ready(data) -> bool:
    try:
        ...
        get_logger().exception(f"Failed 'is_draft_ready' logic")
    return False
...
def should_process_pr_logic(data) -> bool:
    try:
        ...
        get_logger().exception(f"Failed 'should_process_pr_logic'")
    return True
    ...
    async def inner(data: dict):
        ...
        if request.headers.get("X-Gitlab-Token") and secret_provider:
            ...
            try:
                ...
                get_logger().exception(f"Failed to validate secret {request_token}")
...
def handle_ask_line(body, data):
    try:
        ...
        get_logger().exception(f"Failed to handle ask line comment")
    return body
