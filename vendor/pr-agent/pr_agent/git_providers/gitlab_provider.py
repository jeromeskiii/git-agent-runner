...
class GitLabProvider(GitProvider):
    ...
    def __init__(self, merge_request_url: Optional[str] = None, incremental: Optional[bool] = False):
        ...
        try:
            ...
            get_logger().exception(f"Failed to create GitLab instance")
    ...
    def create_or_update_pr_file(self, file_path: str, branch: str, contents="", message="") -> None:
        ...
        try:
            ...
            get_logger().exception(f"Authentication failed while creating/updating file {file_path} in branch {branch}")
            raise
        except (GitlabCreateError, GitlabUpdateError) as e:
            get_logger().exception(f"Permission denied or validation error for file {file_path} in branch {branch}")
            raise
        except Exception as e:
