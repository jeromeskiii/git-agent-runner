...
def get_pr_diff(git_provider: GitProvider, token_handler: TokenHandler,
    ...
    try:
        ...
        get_logger().exception(f"Rate limit exceeded for git provider API. original message ")
        raise
...
def add_ai_metadata_to_diff_files(git_provider, pr_description_files):
    ...
    try:
        ...
        get_logger().exception(f"Failed to add AI metadata to diff files",
...
def add_ai_summary_top_patch(file, full_extended_patch):
    try:
        ...
        get_logger().exception(f"Failed to add AI summary to the top of the patch",
                           artifact={"traceback": traceback.format_exc()})
        return full_extended_patch
