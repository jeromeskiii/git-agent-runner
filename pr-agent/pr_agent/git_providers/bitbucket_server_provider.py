...
class BitbucketServerProvider(GitProvider):
    ...
    def get_repo_settings(self):
        try:
            ...
            get_logger().exception(f"Failed to load .pr_agent.toml file, error")
            return ""
    ...
    def get_diff_files(self) -> list[FilePatchInfo]:
        ...
        if self.bitbucket_api_version is not None and self.bitbucket_api_version >= parse_version("8.16"):
            try:
                ...
                get_logger().exception(f"Failed to get the best common ancestor for PR: {self.pr_url}, \nerror")
                raise e
        else:
    ...
    def publish_inline_comment(self, comment: str, from_line: int, file: str, original_suggestion=None):
        ...
        try:
            ...
            get_logger().exception(f"Failed to publish inline comment to '{file}' at line {from_line}, error")
            raise e
    ...
    def _get_pr(self):
        try:
            ...
            get_logger().exception(f"Failed to get pull request, error")
            raise e
    ...
    def publish_description(self, pr_title: str, description: str):
        ...
        try:
            ...
            get_logger().exception(f"Failed to update pull request, error")
            raise e
