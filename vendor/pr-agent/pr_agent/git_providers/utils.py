...
def validate_pr_url(pr_url: str) -> bool:
    """
    Validate that a PR URL is well-formed and safe to process.
    
    Args:
        pr_url: The PR URL to validate
        
    Returns:
        True if valid, False otherwise
    """
    if not pr_url or not isinstance(pr_url, str):
        get_logger().error("PR URL is empty or not a string")
        return False
    
    pr_url = pr_url.strip()
    if not pr_url:
        get_logger().error("PR URL is empty after stripping whitespace")
        return False
    ...
    try:
        ...
        if parsed.scheme not in ('http', 'https'):
            get_logger().error(f"PR URL has invalid scheme: {parsed.scheme}")
            return False
        
        # Check netloc (domain)
        if not parsed.netloc:
            get_logger().error("PR URL has no domain/netloc")
            return False
        
        # Check for obvious path traversal or suspicious patterns
        if '..' in pr_url or '<' in pr_url or '>' in pr_url:
            get_logger().error("PR URL contains suspicious characters")
            return False
        
        return True
    except Exception as e:
        get_logger().exception(f"Failed to parse PR URL")
        return False
